  ...
  return (
    ...
    >
      <div className="absolute right-4 top-4 flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
        ...
        <form
          action={onDelete}
          onSubmit={(e) => { if (!confirm('Eliminar este curso?')) e.preventDefault() }}
        >
      ...
      <div className="space-y-1 pr-16">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {item.is_in_progress && (
            <span className="inline-flex items-center gap-1 rounded-full bg-accent-500/10 px-2 py-0.5 text-[10px] font-medium text-accent-600 dark:text-accent-400">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted">
          ...
          {item.is_in_progress && item.expected_completion_date && (
            <span>Fin est. {formatDate(item.expected_completion_date)}</span>
          )}
          {!item.is_in_progress && item.completed_at && (
        ...
        {item.description && (
          <p className="pt-2 text-sm leading-relaxed text-muted line-clamp-3">{item.description}</p>
        )}
      </div>
    </article>
  )
